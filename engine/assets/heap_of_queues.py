...
class HeapOfQueues:
    ...
    def pop_from_priority(self, priority, direction='left'):
        if priority not in self.queues or not self.queues[priority]:
            raise IndexError(f"No items in the queue with priority {priority}")

        item = self.queues[priority].popleft() if direction == 'left' else self.queues[priority].pop()
    ...
    def remove_item_from_priority(self, priority, item):
        ...
        try:
            ...
            raise ValueError(f"Item {item} not found in the queue for priority {priority}")
...
if __name__ == "__main__":
    ...
    print(hq.remove_item_from_priority(1, 'task7'))  # Remove 'task7' from priority 1, which should delete the queue
